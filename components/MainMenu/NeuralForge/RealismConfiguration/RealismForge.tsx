...
import { 
  ...
  Sparkles, Crown, PanelLeftClose, PanelLeftOpen, Flower, X, Wind, CloudSun, Zap, Fingerprint, BrainCircuit, MessageCircle, Save, Settings2
} from 'lucide-react';
...
import { MiniNeuralForge } from './MiniNeuralForge/index'; 
...
export const RealismForge: React.FC<RealismForgeProps> = ({ initialData, onSave, onCancel, onScroll }) => {
  ...
  // Parsing extra fields for generic sections
  ...
  // --- PARSING EXTRA SECTIONS FROM DESCRIPTION ---
  useEffect(() => {
      ...
      while ((match = regex.exec(desc)) !== null) {
          newSections[match[1].toLowerCase()] = match[2].trim();
      }
      setParsedSections(newSections);
  }, [formData.id]); // Run once on init

  const updateGenericSection = (section: string, value: string) => {
      const updatedSections = { ...parsedSections, [section]: value };
      setParsedSections(updatedSections);

      // Rebuild Description
      // Preserve main description (before any tags) if it exists, but for Realism chars, usually main description is minimal or we can treat it as 'intro'.
      // Here we append tags to the existing description or rebuild it.
      // Strategy: Keep everything before the first [TAG]
      const currentDesc = formData.description || "";
      const mainSplit = currentDesc.split(/\[([A-Z]+)\]/);
      let baseDesc = mainSplit[0].trim();

      let compiled = baseDesc;
      ...
      setFormData({ ...formData, description: compiled });
  };
  ...
  const compileSystemInstruction = () => {
      ...
      // Add custom sections from description if any
    ...
    >
        ...
        <div className="md:hidden h-14 px-4 border-b border-white/10 flex items-center justify-between bg-zinc-950/90 backdrop-blur-lg z-50 sticky top-0 shrink-0 shadow-lg shadow-black/20">
             ...
             <div className="flex items-center gap-2">
                 <button 
                    onClick={() => setShowMiniForge(!showMiniForge)}
                    className={`p-2 rounded-lg transition-colors ${showMiniForge ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:bg-white/10'}`}
                 >
        ...
        <div className="flex-1 flex overflow-hidden relative z-10 flex-row">
                ...
                <div className="p-2 border-t border-blue-200/30 bg-white/40 backdrop-blur-sm relative z-10 space-y-1 flex flex-col">
                    ...
                    {!showMiniForge && (
                        ...
                        >
                            ...
                            {isSidebarCollapsed && <span className="text-[8px] font-mono font-bold text-blue-600 mt-0.5">{Math.round(coherenceScore)}%</span>}
                        </div>
                    )}
                </div>
            </div>
            ...
            >
                ...
                `}>
                    ...
                    {showMiniForge ? (
                        <div className="h-full w-full relative z-50 flex flex-col shadow-2xl overflow-hidden md:rounded-3xl bg-black/50 backdrop-blur-sm">
                        ...
                        <>
                            ...
                            {/* UNLOCKED SECTIONS: Moral, Social, Duality, Capabilities, Lore, Memory */}
                            {activeSection !== 'identity' && activeSection !== 'visuals' && activeSection !== 'voice' && activeSection !== 'psyche' && activeSection !== 'emotional' && activeSection !== 'scenario' && (
                                <div className="animate-fade-in relative z-10 h-full">
                                    <div className="bg-zinc-900/50 p-1 rounded-2xl border border-white/5 shadow-lg flex flex-col h-[60vh] md:h-auto min-h-[400px]">
                                        <div className="p-4 border-b border-white/5 flex items-center gap-3">
                                            <div className="p-2 bg-blue-500/10 rounded-lg text-blue-400">
                                                {React.createElement(MODULES.find(t => t.id === activeSection)!.icon as any, { size: 16 })}
                                            </div>
                                            <div>
                                                <h3 className="text-xs font-bold text-zinc-200 uppercase tracking-widest">
                                                    {MODULES.find(t => t.id === activeSection)?.labelKey ? t(MODULES.find(t => t.id === activeSection)!.labelKey) : activeSection}
                                                </h3>
                                        ...
                                        <textarea 
                                            ...
                                            onChange={(e) => updateGenericSection(activeSection, e.target.value)}
