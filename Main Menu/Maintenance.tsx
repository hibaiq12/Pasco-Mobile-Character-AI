...
export const MaintenanceModal: React.FC<MaintenanceProps> = ({ onUnlock }) => {
    ...
    useEffect(() => {
        ...
        const checkTime = () => {
            ...
            if (h === 0 && m < 15) {
                // Jika sekarang 00:00 - 00:14, target jam 00:15 hari ini
                targetDate.setHours(0, 15, 0, 0);
            } else if (h < 23 || (h === 23 && m < 15)) {
                // Jika sekarang 04:00 - 23:14, target jam 23:15 hari ini
                targetDate.setHours(23, 15, 0, 0);
            } else {
                // Jika sudah lewat jam operasional terakhir, target jam 00:15 besok
    ...
    const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
        ...
        pastedData.forEach((char, index) => {
            if (index < 6 && !isNaN(Number(char))) newPin[index] = char;
        });
        setPin(newPin);
